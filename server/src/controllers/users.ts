...
export const sendVerificationCode :RequestHandler<unknown,unknown,otpRequestBody,unknown> = async (req,res,next)  => {
    try{
        const emailBody = req.body.email?.toString();
    if(await users.findOne({email:emailBody})
...
export const signUp:RequestHandler<unknown,unknown,signUpBody,unknown> = async (req,res,next) =>{
    ...
    try {
        ...
        if(existingUserName){
            throw createHttpError(409,"Username already taken. Please choose a different user name");
        }
...
export const login:RequestHandler<unknown,unknown,LoginBody,unknown> = async(req,res,next) =>{
    ...
    try {
        ...
        const user = await UserModel.findOne({email:email}).select("+username +email +password").exec();
